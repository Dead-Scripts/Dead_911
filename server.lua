...
RegisterNetEvent('Dead-911:CheckPerms')
AddEventHandler('Dead-911:CheckPerms', function()
    local src = source;
        ...
        for i = 1, #roleList do
            for j = 1, #roleIDs do
                if exports.Dead_Discord_API:CheckEqual(roleList[i], roleIDs[j]) then
                    ...
                    print("[911]" .. GetPlayerName(src) .. " received Dead-911 permissions SUCCESS")
                end
            end
        end
    else
    ...
    if (#args > 0) then 
        if tonumber(args[1]) ~= nil then 
            if locationTracker[tonumber(args[1])] ~= nil then 
                ...
                TriggerClientEvent("Dead-911:SetWaypoint", source, loc[1], loc[2]);
